 ...
 */
package es.littledavity.dynamicfeatures.chorbo_list.list.adapter
...
import es.littledavity.dynamicfeatures.chorbo_list.list.adapter.holders.ChorboViewHolder
import es.littledavity.dynamicfeatures.chorbo_list.list.adapter.holders.ErrorViewHolder
import es.littledavity.dynamicfeatures.chorbo_list.list.adapter.holders.LoadingViewHolder
...
/**
 * Class for presenting chorbos List data in a [RecyclerView], including computing
 * diffs between Lists on a background thread.
 *
 * @see BaseListAdapter
 */
class ChorbosListAdapter @Inject constructor(
    @VisibleForTesting(otherwise = PRIVATE)
    val viewModel: ChorboListViewModel
) : BasePagedListAdapter<ChorboItem>(
    ...
    override fun getItemId(position: Int) =
        when (getItemView(position)) {
            ItemView.CHORBO -> getItem(position)?.id ?: -1L
            ItemView.LOADING -> 0L
            ItemView.ERROR -> 1L
        }
     ...
     */
    fun submitState(newState: ChorboListAdapterState) {
        val oldState = state
        state = newState
